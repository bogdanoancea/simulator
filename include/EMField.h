...
public:
	 ...
	 * the Antenna returned by this method provides the highest power (the power of the field is considered to decrease according to a power-law).
	 * @param p the location where we want to find which Antenna provides the highest power the of electromagnetic field.
	 * @param mnoId the id of the MNO for which we search for the Antenna providing the maximum power. Only antennas belonging to this MNO will be considered during computations.
	...
	/**
	 * Returns a pair made of a pointer to an Antenna object and its signal dominance with the property that in the
	 * location specified by  p, the Antenna returned by this method provides signal with the highest dominance.
	 * The signal dominance in this pair is the computed in location given by p.
	 * @param p indicates the location where we want to find which Antenna provides the highest dominance of the signal.
	 * @param mnoId the id of the MNO for which we search for the Antenna providing the maximum value of the dominance. Only antennas belonging to this MNO will be considered during computations.
	 * @return a pair<Antenna*, double> containing a pointer to the Antenna object that provides a signal with the
	 * highest dominance in location given by p.
	 */
	pair<Antenna*, double> computeMaxDominance(const Point* p, const unsigned long mnoId);
	 ...
	 * @param mnoId the id of the MNO for which we search for the Antenna providing the maximum signal strength.
	 * Only antennas belonging to this MNO will be considered during computations.
	 * @return a pair<Antenna*, double> containing a pointer to the Antenna object that provides a signal with the
	...
	/**
	 * Returns a vector of pairs made up of a pointer to an Antenna object and its power, signal dominance or signal strength.
	 * All the antennas in this vector provides a signal with a power or signal dominance or signal strength greater than
	 * the minimum value provided through the threshold parameter, i.e. this vector contains all antennas that have in
	 ...
	 * @param threshold the lowest limit of the power or signal dominance or signal strength below which the signal is considered to be only
	 * noise, i.e. it defines the limit of the coverage area. Thee actual type of measure (power, dominance or strength is dictate by the parameter connType).
	 ...
	 * @return a vector of pairs made up of a pointer to an Antenna object and its power, signal dominance or signal strength, according to the value
	 * of the connType.
	 * All the antennas in this vector provides a signal with a power, signal dominance or signal strength greater than the value of the threshold parameter.
	 */
	 ...
	 * where the signal dominance, strength or power  is greater than the value of threshold parameter.
	 * @param p the location where we want to check the signal power, dominance or  strength.
	 * @param a pointer to an Antenna object for which we want to check if it covers the location given by the parameter p.
	 * @param threshold the lowest limit of the signal power, dominance or strength below which the signal is considered only noise.
	 * @param connType indicates the mechanism used to set up a connection between an antenna and a mobile phone.
	 ...
	 * The connection likelihood is computed by dividing the signal dominance or strength provided by Antenna a in the location indicated
	 * through p by the sum  of the signal dominance or strength provided by all antennas of an MNO.
	 * @param a a pointer to an Antenna object.
	 * @param p a location on the map.
	 * @return the connection likelihood for Antenna a in location  p.
	 */
	double connectionLikelihood(Antenna* a, const Point * p);

	/**
	 * Computes the sum of the signal dominance given by all antennas belonging to an MNO for all tiles in the reference grid.
	 * The signal dominance is computed in the center of each tile.
	 ...
	 */
	vector<double> sumSignalDominance(const Map* map, const unsigned long mnoID);
