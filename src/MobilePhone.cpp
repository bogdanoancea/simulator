...
MobilePhone::MobilePhone(Map* m, long id, Point* initPosition, Agent* holder, Clock* clock, double powerThreshold, double qualityThreshold) :
	...
	bool MobilePhone::tryConnectNaiveAlgorithm() {
		...
		if (m_connectedTo != nullptr) {
			...
			bool inRange = EMField::instance()->isAntennaInRange(p, m_connectedTo, m_powerThreshold, true);
			if (!inRange) {
		...
		if (connected) {
			...
			vector<pair<Antenna*, double>> antennas = EMField::instance()->getInRangeAntennas(p, m_powerThreshold, true);
